...
<%-- 직접 만든 CSS --%>
<link rel="stylesheet" type="text/css" href="<%=ctxPath %>/resources/css/board/academy/academyList.css" />
...
<%-- 직접만든 javascript --%>
<script type="text/javascript" src="<%=ctxPath %>/resources/js/board/academy/academyList.js" ></script>
  ...
  <div class="container mt-4">
    ...
    <div class="d-flex align-items-center border-top w-100">
      <%-- 학원 사진 --%>
      <div class="academy_img_box mr-2" onclick="location.href='<%=ctxPath %>/boards/${board.id}'">
        <img src="<%=ctxPath %>/resources/images/${board.academyImage}">
      </div>
      ...
      <div class="px-2 py-2 w-100">
        ...
        <%-- 글제목 --%>
        <a href="/boards/${board.id} " class="subject align-items-center my-2">
          ${board.subject}
        </a>
        ...
        <div class="academy_info mt-1">
          <span class="mr-2">문의처 : ${board.phone}</span> 홈페이지 : <a href="${board.homepage}">${board.homepage}</a>
        </div>
    ...
    <div class="d-flex border-top pt-3 justify-content-between">
	  ...
	  <c:if test="${loginMember.userRole == 'ACADEMY' || loginMember.userRole == 'ADMIN'}">
        <button type="button" id="btn_write" class="btn border-0 rounded" onclick="location.href='/boards/academy/new'">
